...
export async function POST(req: Request) {
  try {
    ...
    const taskTexts = Array.isArray(tasks)
      ? tasks.map((t: string) => t.trim()).filter(Boolean)
      : []; //Array.isArray(tasks)でtasksが配列かどうかを判定し配列からtrim()で空白のものを削除tems = ["朝ラン", "  ", "  勉強  "]　をtasks = ["朝ラン", "勉強"]にできる
    ...
    const post = await prisma.post.create({
      data: {
        ...
        tasks: {
          //このtasks:{}とはPostテーブルと関連しているTaskテーブルに同時に作成できる。この段階でpostIdがはいる
    ...
    //errはprisma.post.create(...)が失敗した時にerrが入る
    if (err instanceof Prisma.PrismaClientKnownRequestError && err.code === "P2002") {
      //err.code　codeを使えばprismaが付与したエラー番号を判定できる
...
export async function GET(req: Request) {
  ...
  if (!userId || !targetDate) {
    return NextResponse.json({ error: "migging params" }, { status: 400 });
  }
  ...
  const post = await prisma.post.findUnique({
    where: {
      userId_targetDate: {
        // @@unique([userId, targetDate]) とprismaに定義した場合tsではuserId_targetDateという形でここに入力する.Prisma が userId_targetDateというキーを自動生成します
        userId,
        targetDate: date,
      },
    },
